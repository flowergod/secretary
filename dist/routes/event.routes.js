...
exports.syncToICloud = exports.syncFromICloud = exports.deleteEvent = exports.updateEvent = exports.createEvent = exports.getEvent = exports.listEvents = void 0;
...
const listEvents = async (req, res) => {
    try {
        ...
        const query = {};
        if (rawQuery.date)
            query.date = rawQuery.date;
        if (rawQuery.start_date)
            query.startDate = rawQuery.start_date;
        if (rawQuery.end_date)
            query.endDate = rawQuery.end_date;
        if (rawQuery.category)
            query.category = rawQuery.category;
        if (rawQuery.page)
            query.page = parseInt(rawQuery.page, 10);
        if (rawQuery.page_size)
            query.pageSize = parseInt(rawQuery.page_size, 10);
        const result = await services_1.scheduleService.querySchedules(query);
        sendJson(res, 200, {
            success: true,
            data: {
        ...
        logger_1.logger.error('[Route] listEvents error:', e);
...
const getEvent = async (req, res) => {
    try {
        ...
        const task = await services_1.scheduleService.getSchedule(id);
        if (!task) {
        ...
        logger_1.logger.error('[Route] getEvent error:', e);
        sendJson(res, 500, { success: false, error: { code: 500, message: 'Internal error' } });
    }
};
...
const createEvent = async (req, res) => {
    try {
        ...
        const { title, description, start_date, start_time, end_time, category, is_recurring, recurrence_type, recurrence_rule } = body;
        // 验证必填字段
        if (!title) {
        ...
        const created = await services_1.taskService.create({
            title,
            description,
            start_date,
            start_time,
            ...
            recurrence_type: (recurrence_type || (recurrence_rule ? 'weekly' : 'none')),
            recurrence_rule,
            source: 'api',
        });
        if (!created.success) {
        ...
        logger_1.logger.error('[Route] createEvent error:', e);
...
const updateEvent = async (req, res) => {
    try {
        ...
        const body = await parseBody(req);
        // 验证日程是否存在
        const existing = await services_1.scheduleService.getSchedule(id);
        if (!existing) {
            sendJson(res, 404, { success: false, error: { code: 404, message: 'Schedule not found' } });
            return;
        }
        ...
        logger_1.logger.error('[Route] updateEvent error:', e);
...
const deleteEvent = async (req, res) => {
    try {
        ...
        logger_1.logger.error('[Route] deleteEvent error:', e);
...
const syncFromICloud = async (req, res) => {
    try {
        const body = await parseBody(req);
        const { calendar_id, start_date, end_date } = body;
        const result = await services_1.scheduleService.syncFromICalendar(calendar_id, start_date, end_date);
        sendJson(res, 200, {
            success: true,
            data: {
        ...
        logger_1.logger.error('[Route] syncFromICloud error:', e);
        if (e instanceof icloud_1.ICloudError) {
...
exports.syncFromICloud = syncFromICloud;
// POST /api/events/sync-to-icloud/:id - 手动同步指定日程到 iCloud
const syncToICloud = async (req, res) => {
    try {
        const url = new URL(req.url || '', `http://${req.headers.host}`);
        const id = extractId(url.pathname);
        const task = await services_1.scheduleService.getSchedule(id);
        ...
        const result = await services_1.scheduleService.syncToICalendar(task);
        if (!result.success) {
        ...
        logger_1.logger.error('[Route] syncToICloud error:', e);
        sendJson(res, 500, { success: false, error: { code: 500, message: 'Failed to sync to iCloud' } });
    }
};
exports.syncToICloud = syncToICloud;

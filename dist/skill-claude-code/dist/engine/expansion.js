...
exports.expansionEngine = exports.ExpansionEngine = void 0;
...
class ExpansionEngine {
     ...
     * 医疗复查拓展 - 3个月后提醒
     */
    async generateMedicalFollowup(task) {
        return {
            shouldExpand: true,
            tasks: [{
                    title: `复查：${task.title}`,
     ...
     * 开发进度规划拓展
     */
    async generateDevelopmentPlanning(task) {
...
[
  ...
  {"title": "集成测试", "priority": "medium", "days": 1}
     ...
     * 默认拓展逻辑
     */
    async generateDefaultExpansion(task) {
...
只返回JSON，不要其他内容。`;
     ...
     * 解析 AI 返回的任务列表
     */
    parseTasksFromResponse(content, parentId) {
        try {
            // 尝试提取 JSON 数组
...
exports.expansionEngine = new ExpansionEngine();

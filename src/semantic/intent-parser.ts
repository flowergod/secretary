...
import { IntentType, EntityType, ParsedIntent } from './types';
import { Capability } from './types';
import { logger } from '../shared/logger';

interface LLMIntentClassification {
  intent: string;
  entity_type: string;
...
export class IntentParser {
  // 意图字符串到枚举的映射
  private static INTENT_MAP: Record<string, IntentType> = {
    ...
    'expand_task': IntentType.EXPAND_TASK,
    'other': IntentType.OTHER,
  };

  // 实体类型字符串到枚举的映射
  private static ENTITY_TYPE_MAP: Record<string, EntityType> = {
    'task': 'task',
    'event': 'event',
    'calendar': 'calendar',
  };
  ...
  parseIntentClassification(
    llmOutput: unknown,
    rawInput: string
  ): { intent: ParsedIntent; missingParams: string[] } {
    // 验证LLM输出格式
    if (!this.isValidIntentOutput(llmOutput)) {
      logger.warn('[IntentParser] Invalid LLM output format:', llmOutput);
      return this.createDefaultResult(rawInput);
    }

    const output = llmOutput as LLMIntentClassification;

    // 转换意图
    const intent = IntentParser.INTENT_MAP[output.intent] || IntentType.OTHER;
    ...
    logger.debug('[IntentParser] Parsed intent:', parsedIntent);

    return { intent: parsedIntent, missingParams };
  }

  // 验证LLM输出是否有效
  private isValidIntentOutput(output: unknown): boolean {
  ...
  // 创建默认结果（当解析失败时）
  private createDefaultResult(rawInput: string): { intent: ParsedIntent; missingParams: string[] } {
    return {
      intent: {
  ...
  private normalizeDate(dateStr: string): string {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      ...
      if (year < todayYear ||
        ...
        logger.info(`[IntentParser] Date ${date} is in the past, treating as today`);

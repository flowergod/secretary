...
import { SemanticResult, ParsedIntent, LLMConfig, ConfirmationOption, ReferenceResolutionResult } from './types';
import { ContextManager, Context, getContextManager } from './context-manager';
import { getSemanticLogger, SemanticLogger, SemanticLogEntry } from './semantic-logger';
import { getTraceLogger, TraceLogger } from './trace-logger';
...
export class SemanticService {
  ...
  constructor() {
    ...
    this.llmService = new LLMService(llmConfigs);

    // 初始化其他组件
    this.promptManager = new PromptManager();
    this.dispatcher = new CapabilityDispatcher();
    this.intentParser = new IntentParser(this.dispatcher.getAllCapabilities());
    this.contextManager = getContextManager();
    this.semanticLogger = getSemanticLogger();
    this.traceLogger = getTraceLogger();
  }

  // 加载LLM配置
  private loadLLMConfigs(): LLMConfig[] {
    ...
    if (aiConfig?.primary) {
      configs.push({
        provider: (aiConfig.primary.provider || 'minimax') as 'volcano' | 'minimax' | 'openai',
    ...
    if (aiConfig?.fallback) {
      configs.push({
        provider: (aiConfig.fallback.provider || 'minimax') as 'volcano' | 'minimax' | 'openai',
    ...
    logger.info(`[SemanticService] Loaded ${configs.length} LLM config(s)`);
    return configs;
  }
  ...
  async understand(
    ...
    let logEntry: SemanticLogEntry | null = null;

    try {
      logger.info(`[SemanticService] Understanding: "${userInput}"`);

      // 检查是否存在 pending_confirmation 的上下文
      const pendingContexts = this.contextManager.getPendingContexts();
      logger.info(`[SemanticService] Pending contexts count: ${pendingContexts.length}`);

      if (pendingContexts.length > 0) {
        const pendingContext = pendingContexts[0];
        logger.info(`[SemanticService] Has pending context: ${pendingContext.id}, question: ${pendingContext.confirmationQuestion}`);
        ...
        if (contextualResult.wantsNewTask) {
          logger.info(`[SemanticService] User wants to start new task, cancelling pending context`);
          this.contextManager.cancelContext(pendingContext.id);
          // 继续执行下面的新任务识别逻辑
        }
      } else {
        logger.info(`[SemanticService] No pending context found, proceeding with new intent classification`);
      }
      ...
      if (referenceResult.found && referenceResult.resolvedTask) {
        logger.info(`[SemanticService] Resolved reference to: ${referenceResult.resolvedTask.title} (${referenceResult.resolvedTask.taskId})`);
      }
      // ========== 指代词解析结束 ==========
      ...
      // 记录语义日志（意图识别阶段）
      logEntry = this.semanticLogger.createLog(
      ...
      if (intentResult.intent.needsConfirmation) {
        ...
        if (intentResult.intent.intent === 'delete_task') {
          ...
          if (events.length > 0) {
            // 显示事件列表供选择
            const options = events.slice(0, 5).map((evt: any, i: number) => {
        ...
        const confirmation = await this.generateConfirmation(
          userInput,
          intentResult.intent,
          intentResult.missingParams
        );
        this.traceLogger.endSpan(traceId, confirmationSpan!);
        ...
        const context = this.contextManager.createContext(
          ...
          confirmation.options?.map((opt, i) => ({
            id: `opt_${i + 1}`,
            label: opt,
            type: 'task',
          })),
        ...
        logger.info(`[SemanticService] Created context: ${context.id}`);
      ...
      this.semanticLogger.updateExecution(logEntry.id, {
        executed: true,
        capabilityId: dispatchResult.capabilityId,
        taskId: dispatchResult.data as string,
        action: 'completed',
        durationMs: executionDuration,
      });

      // 更新近期操作记录
      this.recordRecentMention(intentResult.intent, dispatchResult.data as string);
      ...
      logger.error('[SemanticService] Understanding failed:', error);
  ...
  async confirm(
    ...
    try {
        ...
        if (!context.confirmationOptions || context.confirmationOptions.length === 0) {
          ...
          if (selectedOption && simpleConfirmPatterns.includes(selectedOption.toLowerCase())) {
            ...
            if (dispatchResult.data) {
              this.recordRecentMention(context.intent, dispatchResult.data as string);
            }
      ...
      logger.error('[SemanticService] Confirmation execution failed:', error);
      this.traceLogger.endTrace(traceId, {
  ...
  // 根据关键词查询匹配的任务
  private async findMatchingTasks(params: Record<string, unknown>): Promise<Task[]> {
    // 尝试多种可能的关键词字段
    ...
    if (!keyword) {
      logger.info(`[SemanticService] findMatchingTasks: no keyword found`);
      return [];
    }

    logger.info(`[SemanticService] findMatchingTasks: searching with keyword="${keyword}"`);

    // 查询所有任务
    const result = await taskService.list({ page_size: 100 });

    if (!result.success || !result.data) {
      logger.info(`[SemanticService] findMatchingTasks: query failed`);
      return [];
    }

    logger.info(`[SemanticService] findMatchingTasks: total tasks=${result.data.items.length}`);

    // 使用 LLM 做语义相似度匹配
    const similarTasks = await this.findSimilarTasksByLLM(keyword, result.data.items);
    logger.info(`[SemanticService] findMatchingTasks: LLM matched ${similarTasks.length} tasks`);

    return similarTasks;
  }

  // 使用 LLM 评估语义相似度
  private async findSimilarTasksByLLM(query: string, tasks: Task[]): Promise<Task[]> {
    if (tasks.length === 0) {
      return [];
    }
...
请以JSON格式返回：
{
  ...
  "reasoning": "判断理由"
    ...
    try {
      const llmResponse = await this.llmService.completeJson<{
        related_indices: number[];
        reasoning: string;
      }>(prompt, '', { temperature: 0.3 });

      logger.info(`[SemanticService] findSimilarTasksByLLM: reasoning=${llmResponse.reasoning}`);
      ...
      logger.info(`[SemanticService] findSimilarTasksByLLM: found ${relatedTasks.length} related tasks: ${relatedTasks.map(t => t.title).join(', ')}`);

      return relatedTasks;
    } catch (error) {
      logger.error(`[SemanticService] findSimilarTasksByLLM failed:`, error);
      // LLM 失败时回退到简单的关键词匹配
  ...
  // 根据日期查询事件（用于删除确认）
  private async findEventsForDate(dateStr: string): Promise<Task[]> {
    if (!dateStr) {
      return [];
    }
  ...
  private async findEventsForDateRange(days: number): Promise<Task[]> {
  ...
  // 查询符合条件的任务（用于删除/更新确认）
  private async findTasksForConfirmation(params: Record<string, unknown>): Promise<{ tasks: Task[]; message: string }> {
  ...
  // 解析用户输入中的指代词
  private async resolveReference(userInput: string): Promise<ReferenceResolutionResult> {
    // 获取近期操作记录
    ...
    try {
      ...
      logger.info(`[SemanticService] Reference resolution: type=${llmResponse.type}, reasoning=${llmResponse.reasoning}`);
      ...
      logger.error('[SemanticService] Reference resolution failed:', error);
      return { found: false, resolvedTask: null, resolvedUserInput: userInput };
    }
  }

  // 使用 LLM 理解上下文中的用户输入
  private async understandContextualInput(
    userInput: string,
    context: Context
  ): Promise<{
    handled: boolean;
    ...
    // 使用 LLM 理解
    const { system, user } = this.promptManager.renderContextualUnderstanding(
      context.confirmationQuestion || '请确认',
    ...
    try {
      ...
      logger.info(`[SemanticService] Contextual understanding: type=${llmResponse.type}, reasoning=${llmResponse.reasoning}`);
      ...
      switch (llmResponse.type) {
        ...
        case 'new_task':
          return { handled: false, wantsNewTask: true, result: {} as SemanticResult };
      }
    } catch (error) {
      logger.error('[SemanticService] Contextual understanding failed:', error);
    }
  ...
  ): Promise<{ intent: ParsedIntent; missingParams: string[] }> {
    // 渲染提示词（传入解析上下文以帮助 LLM 理解）
    const { system, user } = this.promptManager.renderIntentClassification(
      userInput,
      this.dispatcher.getAllCapabilities(),
      context ? {
        resolvedTaskId: context.resolvedTaskId as string | undefined,
        resolvedRecordId: context.resolvedRecordId as string | undefined,
        resolvedTaskTitle: context.resolvedTaskTitle as string | undefined,
      } : undefined
    );
  ...
  private async generateConfirmation(
    ...
    const { system, user } = this.promptManager.renderConfirmation(
      userInput,
      intent.intent,
      formattedParams,
      missingParams
    ...
    try {
      ...
      logger.warn('[SemanticService] Failed to generate confirmation, using fallback');
  ...
  // 格式化参数中的日期，添加正确的星期
  private formatParametersWithDate(params: Record<string, unknown>): Record<string, unknown> {
  ...
  // 记录近期操作
  private recordRecentMention(intent: ParsedIntent, taskId: string): void {
    // 根据意图类型确定操作类型
    let operation: 'create' | 'modify' | 'delete' | 'complete' | 'query' = 'query';
    ...
    // 获取任务标题
    const title = (intent.parameters.title as string) ||
                   (intent.parameters.resolvedTaskTitle as string) ||
                   `任务_${taskId.substring(0, 8)}`;

    // 从参数中获取 recordId
    const recordId = (intent.parameters.recordId as string) ||
                     (intent.parameters.resolvedRecordId as string) ||
                     taskId;

    this.contextManager.addMention(taskId, recordId, title, operation);
    logger.info(`[SemanticService] Recorded mention: ${title} (${operation})`);
  }
